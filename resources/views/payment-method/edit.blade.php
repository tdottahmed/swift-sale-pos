<x-layouts.master>
    <x-data-display.card>
        <x-slot name="heading">
            {{ __('Edit Your Payment Method') }}
        </x-slot>
        <x-slot name="body">
            <form action="{{ route('payment-method.update', $paymentMethod->id) }} " method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-3">
                    <label for="title">Title:</label>
                    <input type="text" class="form-control" name="title" id="title" value="{{ $paymentMethod->title }}">
                </div>
